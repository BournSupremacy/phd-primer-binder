#!/usr/bin/env bash
# Submit the three pipeline steps as SLURM jobs, chained with
# --dependency=afterok so each only starts once the previous one has
# finished successfully. This is the sbatch equivalent of run_pipeline.sh -
# see README.md "Running on a SLURM cluster" for when to use this vs.
# running the scripts/ directly.
#
# Usage:
#   slurm/submit_pipeline.sh --run-dir /path/to/MiSeqRun [--force] [--ref data/binders.fasta]
#
# Run from the repo root (paths in the .sbatch files are relative to it).
...
demux_id=$(sbatch --parsable slurm/00_bcl_to_fastq.sbatch "${demux_args[@]}")
echo "Submitted demux job $demux_id"

qc_id=$(sbatch --parsable --dependency=afterok:"$demux_id" slurm/01_qc_trim.sbatch)
echo "Submitted QC/trim job $qc_id (after $demux_id)"

align_id=$(sbatch --parsable --dependency=afterok:"$qc_id" slurm/02_align_and_count.sbatch --ref "$REF")
